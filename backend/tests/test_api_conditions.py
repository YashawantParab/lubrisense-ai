"""Phase 13 condition-intelligence API: tenant scoping, contract shape, machine-not-found
handling. A fresh, sensor-less machine's `/latest` succeeds at 200 with
`condition_type=INSUFFICIENT_EVIDENCE`, never a crash."""
...
def test_latest_machine_not_found(client: TestClient, api_tenant: Tenant) -> None:
    ...
    response = client.get(f"/api/v1/conditions/machines/{uuid.uuid4()}/latest", headers=headers)
    assert response.status_code == 404


def test_history_machine_not_found(client: TestClient, api_tenant: Tenant) -> None:
    headers = {"X-Tenant-ID": str(api_tenant.id)}
    response = client.get(f"/api/v1/conditions/machines/{uuid.uuid4()}/history", headers=headers)
    assert response.status_code == 404
...
def test_cross_tenant_machine_is_not_found(client: TestClient) -> None:
    ...
    response = client.get(f"/api/v1/conditions/machines/{machine_id}/latest", headers=headers_b)
    assert response.status_code == 404
...
def test_history_returns_empty_list_for_fresh_machine(
    ...
    headers = {"X-Tenant-ID": str(api_tenant.id)}
    response = client.get(f"/api/v1/conditions/machines/{machine_id}/history", headers=headers)
    assert response.status_code == 200
    assert response.json() == []


def test_latest_on_fresh_machine_is_insufficient_evidence_not_a_crash(
    client: TestClient, api_tenant: Tenant
) -> None:
    machine_id = asyncio.run(_committed_machine(api_tenant))
    headers = {"X-Tenant-ID": str(api_tenant.id)}
    response = client.get(f"/api/v1/conditions/machines/{machine_id}/latest", headers=headers)
    assert response.status_code == 200
    body = response.json()
    assert body["condition_type"] == "INSUFFICIENT_EVIDENCE"
...
def test_latest_persists_and_history_reflects_it(client: TestClient, api_tenant: Tenant) -> None:
    ...
    latest_response = client.get(
        f"/api/v1/conditions/machines/{machine_id}/latest", headers=headers
    )
    assert latest_response.status_code == 200

    history_response = client.get(
        f"/api/v1/conditions/machines/{machine_id}/history", headers=headers
    )
