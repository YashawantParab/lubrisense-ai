...
type MLView = "fleet" | "performance" | "governance";

const ML_TABS: { key: MLView; label: string }[] = [
  { key: "fleet", label: "Fleet Evidence" },
  { key: "performance", label: "Model Performance" },
  { key: "governance", label: "Model Governance" },
];
...
function MLPageInner() {
  ...
  const view = (searchParams.get("tab") as MLView | null) ?? "fleet";
  const setView = (value: MLView) => {
    const params = new URLSearchParams(searchParams.toString());
    if (value === "fleet") params.delete("tab");
    else params.set("tab", value);
    const query = params.toString();
    router.replace(query ? `${pathname}?${query}` : pathname, { scroll: false });
  };

  // Default selector prioritizes assets WITH ML evidence — a no-evidence machine only
  // enters the primary selector when the user deliberately asks for "All assets"
  // (Enterprise Product Rebuild Pass 2 §8) — a no-evidence machine must never look like a
  // scored "normal" reading by silently sitting in the same list.
  const showAllAssets = searchParams.get("scope") === "all";
  const setShowAllAssets = (value: boolean) => {
    const params = new URLSearchParams(searchParams.toString());
    if (value) params.set("scope", "all");
  ...
  // A deep link (e.g. from Fleet) to a specific no-evidence machine must still resolve —
  // union it in rather than let the selector silently show a value with no matching
  // option.
  const selectableMachines = useMemo(
    ...
    [showAllAssets, machines, machinesWithEvidence, effectiveMachineId],
  );
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-10 px-6 py-10 lg:px-10">
      ...
      <WorkspaceTabs tabs={ML_TABS} active={view} onChange={setView} />
      ...
      {view === "fleet" && (
        <>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <label className="flex items-center gap-1.5 text-xs text-zinc-500 dark:text-zinc-400">
                  <input
                    ...
                    onChange={(event) => setShowAllAssets(event.target.checked)}
                  />
                  All assets
                </label>
                  ...
                  >
                    {selectableMachines.map((machine) => (
                      <option key={machine.id} value={machine.id}>
                        {machine.name}
                        {(resultsByMachine.get(machine.id)?.length ?? 0) === 0
                          ? " (no ML evidence)"
                          : ""}
                      </option>
                    ))}
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      {!effectiveClassifier ? (
                        ...
                        <ClassificationCard
                          ...
                          includedInCondition={
                            selectedCondition?.ml_result_ids.includes(effectiveClassifier.id) ??
                            false
                          }
                        />
                      )}
                ...
                {selectedCondition && (
                  <div className="border-t border-zinc-100 pt-5 dark:border-zinc-800">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
                      ...
                      {selectedCondition && (
                        <ConfidenceBadge value={selectedCondition.confidence} />
                      )}
          ...
          <SectionCard title="Fleet ML evidence">
            <DataState
              isPending={fleetML.isPending}
              isError={fleetML.isError}
              error={fleetML.error}
            >
                  ...
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-zinc-200 text-xs text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                        ...
                        <th className="py-1.5 pr-4 font-medium">Site / Area</th>
                        <th className="py-1.5 pr-4 font-medium">Anomaly evidence</th>
                        <th className="py-1.5 pr-4 font-medium">Classification evidence</th>
                        <th className="py-1.5 pr-4 font-medium">Data readiness</th>
                        <th className="py-1.5 pr-4 font-medium">Model maturity</th>
                        <th className="py-1.5 pr-4 font-medium">Operational influence</th>
                      </tr>
                    </thead>
                    <tbody>
                      {machinesWithEvidence.map((machine) => {
                        ...
                        const classification =
                          results.find(
                            (r) => r.model_id === "FAILURE_CLASSIFICATION_BASELINE_V1",
                          ) ?? results.find((r) => r.result_kind === "CLASSIFICATION");
                        const condition = conditionByMachine.get(machine.id);
                        const primaryModelId =
                          classification?.model_id ?? anomaly?.model_id ?? null;
                        const primaryModelStatus = primaryModelId
                          ? (modelStatusById.get(primaryModelId) ?? null)
                          : null;
                        const influencesCondition = Boolean(
                          condition &&
                          ((anomaly && condition.ml_result_ids.includes(anomaly.id)) ||
                            (classification &&
                              condition.ml_result_ids.includes(classification.id))),
                        );
                        return (
                          <tr
                            key={machine.id}
                          ...
                          >
                            ...
                            <td className="py-2 pr-4">
                              {primaryModelStatus ? (
                                <ModelStatusBadge value={primaryModelStatus} />
                              ) : (
                                <span className="text-xs text-zinc-400 dark:text-zinc-600">—</span>
                              )}
                            </td>
                            <td className="py-2 pr-4">
                              <StatusPill tone={influencesCondition ? "ok" : "neutral"}>
                                {influencesCondition
      ...
      {(view === "performance" || view === "governance") && (
        <DataState isPending={models.isPending} isError={models.isError} error={models.error}>
          {sortedModels.length === 0 ? (
            ...
            <div className="flex flex-col gap-6">
              ...
              {view === "performance" && detail.data && (
                <SectionCard
                  title={`${modelDisplayName(detail.data.model_id)} — performance`}
              ...
              {view === "governance" && detail.data && (
                <SectionCard
                  title={`${modelDisplayName(detail.data.model_id)} — governance`}
                  actions={<ModelStatusBadge value={detail.data.status} />}
                >
                  <div className="flex flex-col gap-5">
                    <LifecycleExplainer
                      status={detail.data.status}
                      modelId={detail.data.model_id}
                    />

                    {detail.data.promotion_history.length > 0 ? (
                      <div className="border-t border-zinc-100 pt-4 dark:border-zinc-800">
                        <p className="mb-2 text-xs font-medium text-zinc-500 dark:text-zinc-400">
                          Lifecycle decisions — last evaluated / promotion history
                        </p>
                      ...
                      <p className="border-t border-zinc-100 pt-4 text-xs text-zinc-500 italic dark:border-zinc-800 dark:text-zinc-400">
                        No lifecycle promotion/demotion has been recorded for this model yet — it
                        has held its current stage since registration.
                      </p>
                    )}
